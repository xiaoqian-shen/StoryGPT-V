...
def plot_overlay_heat_map(im, heat_map, word=None, out_file=None, crop=None, color_normalize=True, ax=None, word_idx=None, caption=None):
    if ax is None:
        plt.clf()
    ...
    with auto_autocast(dtype=torch.float32):
        ...
        if color_normalize:
            plt_.imshow(heat_map.squeeze().cpu().numpy(), cmap='coolwarm')
        else:
            heat_map = heat_map.clamp_(min=0, max=1)
            plt_.imshow(heat_map.squeeze().cpu().numpy(), cmap='coolwarm', vmin=0.0, vmax=1.0)
...
class WordHeatMap:
    ...
    def plot_overlay(self, image, word_idx=None, caption=None, out_file=None, color_normalize=True, ax=None, **expand_kwargs):
    ...
    def expand_as(self, image, absolute=False, threshold=None, plot=False, **plot_kwargs):
        ...
        # print(im.shape)
        # im = F.interpolate(im.float().detach(), size=(image.size[0], image.size[1]), mode='nearest')
...
class GlobalHeatMap:
    ...
    def compute_word_heat_map(self, word: str, word_idx: int = None, offset_idx: int = 0) -> WordHeatMap:
        ...
        return [WordHeatMap(self.heat_maps[merge_idx], word, word_idx) for merge_idx in merge_idxs]
