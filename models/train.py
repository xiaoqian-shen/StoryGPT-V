...
from models.data import get_data_loader, ImageDataset
...
def train():
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("story", config=vars(args), init_kwargs={"wandb": {"entity": "xiaoqian-shen"}})
    ...
    print("Total trainable parameter", param_count)
    total_batch_size = (
        args.train_batch_size
        * accelerator.num_processes
        * args.gradient_accumulation_steps
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model), torch.backends.cuda.sdp_kernel(
                ...
                if "reg_loss" in return_dict:
                    avg_regularization_loss = accelerator.gather(
                        return_dict["reg_loss"].repeat(args.train_batch_size)
                    ).mean()
                    regularization_loss += (
                        avg_regularization_loss.item() / args.gradient_accumulation_steps
                    )
            ...
            if "reg_loss" in return_dict:
                logs["l_reg"] = return_dict["reg_loss"].detach().item()
