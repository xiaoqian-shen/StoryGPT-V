...
class CustomImageEncoder(CLIPPreTrainedModel):
    @staticmethod
    def from_pretrained(
...
def fuse_object_embeddings(
    inputs_embeds, # [bsz, seq_len, dim]
    image_token_mask, # [bsz, seq_len]
    object_embeds,
    num_objects, # [bsz, 1]
    fuse_fn=torch.add,
):
    object_embeds = object_embeds.to(inputs_embeds.dtype)
    batch_size, max_num_objects = object_embeds.shape[:2] # [bsz, max_n_object, 1, 768]
    seq_length = inputs_embeds.shape[1] 
    flat_object_embeds = object_embeds.view(
        -1, object_embeds.shape[-2], object_embeds.shape[-1]
    ) # [bsz * max_n_object, 1, 768]
    ...
    inputs_embeds = inputs_embeds.view(-1, inputs_embeds.shape[-1]) # [bsz * seq_len,dim]
    image_token_mask = image_token_mask.view(-1)
    valid_object_embeds = valid_object_embeds.view(-1, valid_object_embeds.shape[-1])

    # slice out the image token <|image|> embeddings
    image_token_embeds = inputs_embeds[image_token_mask] # [n_object, dim]
...
class CustomTextEncoder(CLIPPreTrainedModel):
    ...
    def from_pretrained(model_name_or_path, **kwargs):
        ...
        return CustomTextEncoder(text_model)
...
def unet_store_cross_attention_scores(unet, attention_scores, layers=5):
    ...
    applicable_layers = UNET_LAYER_NAMES[start_layer:end_layer] # ['down_blocks.1', 'down_blocks.2', 'mid_block', 'up_blocks.1', 'up_blocks.2']
...
class StoryModel(nn.Module):
    def __init__(self, text_encoder, image_encoder, vae, unet, args):
        ...
        self.enable_reg_loss = args.enable_reg_loss
        self.reg_weight = args.reg_weight
        self.operate_layers = args.operate_layers
        ...
        self.postfuse_module = FuseModule(embed_dim)

        if self.enable_reg_loss:
            self.cross_attention_scores = {}
            self.unet = unet_store_cross_attention_scores(
                self.unet, self.cross_attention_scores, self.operate_layers
            )
    ...
    def from_pretrained(args):
        text_encoder = CustomTextEncoder.from_pretrained(
        ...
        image_encoder = CustomImageEncoder.from_pretrained(
            args.image_encoder_name_or_path,
        )

        return StoryModel(text_encoder, image_encoder, vae, unet, args)
    ...
    def forward(self, batch, noise_scheduler):
        ...
        if self.enable_reg_loss:
            ...
            loss = self.reg_weight * reg_loss + denoise_loss
