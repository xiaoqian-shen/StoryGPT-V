...
def validate(val_loader, model, tokenizer, criterion, epoch, args):
    ...
    feature_extractor = utils.get_feature_extractor_for_model(args.visual_model, image_size=args.image_size,
                                                              train=False)
            ...
            for i, (image_paths, images, captions, clip_emb, _) in tqdm.tqdm(enumerate(loader), position=0,
                ...
                for model_mode in model_modes:
                    ...
                    (model_output, full_labels, last_embedding, _, visual_embs, visual_embs_norm,
                     input_embs_norm, _, labels) = model(images, mode=model_mode, captions=captions)  # (N, T, C)
                    ...
                    if model_mode == 'captioning':
                        ...
                        ce_losses.update(loss.item(), len(images))
                    elif model_mode == 'retrieval':
                        if args.distributed:
                            ...
                            all_last_embedding = [torch.zeros_like(last_embedding) for _ in
                            ...
                            start_idx = args.rank * len(images)
                    ...
                    if model_mode == 'captioning':
                        ...
                        input_embs = model.module.model.get_visual_embs(images,
                                                                        mode='captioning')  # (2, n_visual_tokens, D)
                        if args.input_prompt is not None:
                            print(f'Adding prefix "{args.input_prompt}" to captioning generate=True.')
                            prompt_ids = tokenizer(args.input_prompt, add_special_tokens=True,
                        ...
                        if i == 0:
                            ...
                            # input_ids = labels[:, :3]  # Use first 3 tokens as initial prompt for generation.
                            ...
                            # input_emb_before = model.module.model.input_embeddings(tokenizer('Image: <img>', return_tensors="pt", add_special_tokens=True).input_ids[0].to(visual_emb.device))
                            # input_emb_after = model.module.model.input_embeddings(tokenizer('</img> Caption: ', return_tensors="pt").input_ids[0].to(visual_emb.device))
                            # input_emb_before = input_emb_before.unsqueeze(0).repeat_interleave(batch_size, dim=0)
                            # input_emb_after = input_emb_after.unsqueeze(0).repeat_interleave(batch_size, dim=0)
                            # print(input_emb_before.shape, input_emb_after.shape, input_embs.shape)
                            # input_embs = torch.cat([input_emb_before, input_embs, input_emb_after], dim=1)
                            generated_ids, _, _ = model(input_embs, generate=True, num_words=num_words, temperature=0.0,
                                                        top_p=1.0)
                            generated_captions = tokenizer.batch_decode(generated_ids, skip_special_tokens=False)
                            gt_captions = tokenizer.batch_decode(labels, skip_special_tokens=False)
                    else:
                        raise NotImplementedError
                    ...
                    vis_emb_norm.update(visual_embs_norm.item(), len(images))
