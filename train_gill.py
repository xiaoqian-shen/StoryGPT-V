...
def parse_args(args):
  ...
  parser.add_argument('--dataset-dir', default='/home/shenx/datasets/flintstones_data', type=str,
...
def main_worker(gpu, ngpus_per_node, args):
  ...
  model = models.GILL(tokenizer, model_args, emb_matrix=train_dataset.clip_embs)
  ...
  for epoch in range(args.start_epoch, args.epochs):
    ...
    if not args.multiprocessing_distributed or (args.multiprocessing_distributed
      ...
      if is_best:
        ...
        finetuned_tokens = checkpoint['state_dict']['model.input_embeddings.weight'][-args.num_tokens:, :].detach().clone()
...
def train(train_loader, model, tokenizer, criterion, optimizer, epoch, scheduler, args):
  ...
  # for i, (_, images, ret_tokens, ret_caption_len, gen_tokens, gen_caption_len, clip_emb) in enumerate(train_loader):
  for i, (_, images, captions, clip_emb) in enumerate(train_loader):
    ...
    for model_mode in model_modes:
      ...
      if model_mode == 'retrieval':
        ...
        cont_losses.update(loss.item(), len(images))

        # measure accuracy and record loss
        top1_caption.update(caption_acc1[0], len(images))
        top5_caption.update(caption_acc5[0], len(images))
        top1_image.update(image_acc1[0], len(images))
        top5_image.update(image_acc5[0], len(images))
      elif model_mode == 'generation':
      ...
      if model_mode == 'retrieval':
        ret_vis_emb_norm.update(visual_embs_norm.item(), len(images))
      elif model_mode == 'captioning':
        cap_vis_emb_norm.update(visual_embs_norm.item(), len(images))
