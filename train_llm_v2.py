...
def parse_args(args):
    ...
    parser.add_argument("--operate_layers", type=int, default=5)
    parser.add_argument("--mask_loss", action="store_true")
    parser.add_argument("--mask_loss_prob", type=float, default=0.5)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    storyldm = StoryModel.from_pretrained(args)
    ckpt_name = "pytorch_model.bin"

    storyldm.load_state_dict(
        torch.load(Path(args.finetuned_model_path) / ckpt_name, map_location="cpu"), strict=False
    )

    storyldm = storyldm.to(dtype=weight_dtype).cuda()
    storyldm.requires_grad_(False)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, storyldm, pipe, tokenizer, criterion, optimizer, epoch, scheduler, args)
...
def train(train_loader, model, storyldm, pipe, tokenizer, criterion, optimizer, epoch, scheduler, args):
    ngpus_per_node = torch.cuda.device_count()
    batch_time = utils.AverageMeter('Time', ':6.3f')
    data_time = utils.AverageMeter('Data', ':6.3f')
    losses = utils.AverageMeter('Loss', ':.4e')
    ce_losses = utils.AverageMeter('CeLoss', ':.4e')
    image_losses = utils.AverageMeter('ImageLoss', ':.4e')
    align_losses = utils.AverageMeter('AlignLoss', ':.4e')
    ...
    progress = utils.ProgressMeter(
        ...
        [batch_time, losses, ce_losses, image_losses, align_losses],
    ...
    for i, (_, images, captions, clip_emb, pred_image) in enumerate(train_loader):
        ...
        if torch.cuda.is_available():
            ...
            pred_image = pred_image.cuda(args.gpu, non_blocking=True)

        if args.precision == 'fp16':
            images = images.half()
            pred_image = pred_image.half()
        elif args.precision == 'bf16':
            images = images.bfloat16()
            pred_image = pred_image.bfloat16()
        ...
        for model_mode in model_modes:
            ...
            align_loss = losses_utils.l2_loss(clip_emb, last_embedding)  # (N,)
            with torch.no_grad():
                latents = storyldm.vae.encode(pred_image).latent_dist.sample()
                latents = latents * storyldm.vae.config.scaling_factor
            ...
            target = noise
            model_pred = storyldm.unet(noisy_latents, timesteps, last_embedding).sample
            image_loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")

            # gen_loss = args.gen_loss_scale * align_loss.mean() + image_loss.mean()
            gen_loss = image_loss.mean()
            loss += gen_loss
            image_losses.update(image_loss.item(), len(images))
            align_losses.update(align_loss.mean().item(), len(images))

            wandb.log({"image_loss": image_loss.mean()})
            wandb.log({"align_loss": align_loss.mean()})
